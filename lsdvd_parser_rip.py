...
def rip(episodes):
    name   = input('What is the name of the serie ?')
    season = input('Which season is it ?')
    disk   = input('Which disk is it ?')
    i      = input('What is the number of the first episode ?')

    with open(os.devnull, mode='w') as f:
        for e in episodes:    
            subprocess.call(
            ['HandBrakeCLI', '-i', '/dev/dvd', '-t', '1', '-o',
            name + '.Season' + season + '.Disk' + disk + '.Episode' + '0' + i +'.mkv',
            '--cfr', '-r', '25'])
            i += 1
